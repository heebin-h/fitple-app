...
>
> 🚨 **본 가이드를 읽기 전에 `docs/SOURCE_INVENTORY.md`의 해당 Phase 섹션을 먼저 보고
> 모든 소스 파일(Android Kotlin/XML/drawable + designs/PNG)을 Read 도구로 직접 읽어야 함.**
> 인벤토리의 체크 박스가 모두 ✅ 가 아닌 상태에서 코드 변경 = 누락 → 사후 수정. 절대 금지.
